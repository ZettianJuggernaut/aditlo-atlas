...
{
	if(device_mouse_x(0) >= x - sprite_get_width(sprite_index)/2 && device_mouse_x(0) <= x)
	{
...
else if(device_mouse_check_button(0,mb_left))
{
	...
	{
		if(device_mouse_x(0) < x - sprite_get_width(sprite_index)/2 || device_mouse_x(0) > x || device_mouse_y(0) < y - sprite_get_height(sprite_index)/2 || device_mouse_y(0) > y + sprite_get_height(sprite_index)/2)
		{
			image_xscale = 1;
			image_yscale = 1;
		}
	...
	if(device_mouse_x(0) >= x - sprite_get_width(sprite_index)/2 && device_mouse_x(0) <= x)
	{
		if(device_mouse_y(0) >= y - sprite_get_height(sprite_index)/2 && device_mouse_y(0) <= y + sprite_get_height(sprite_index)/2)
		{
			if(image_xscale == 1.01)
			{
				...
				ini_open("save.ini");
				ini_write_real("achiev","dreamworld",global.dream_world);
				ini_close();
				repeat(5)
				{
				...
				instance_destroy(obj_timer_medal_right);
				instance_destroy();
			}
		}
	}
